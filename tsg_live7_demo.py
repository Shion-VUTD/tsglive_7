...
各チャレンジごとに30秒のシンキングタイムをとる（つまり、1問あたり4分程度）
...
*   出題する問題の画像（1問分5枚、予選2試合・決勝1試合の合計13問分）をフォルダにして事前配布
*   画像は全て3×224×224
*   フォルダは形式(いわゆるREADME)のみ公開しておく（パスワードでロック）。サンプルフォルダで事前練習も
*   企画開始10分前にパスワード公開

・出力
*   シンキングタイムは30秒しかないので、事前10分以内に、予選から決勝までの合計65枚を同時に実行する必要がある
*   出力形式は
